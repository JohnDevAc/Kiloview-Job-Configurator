...
Prepared 0.8.13 (Main) / 0.8.13-dev.1 (Development) with QA S1/S2 and the reviewed Windows maintenance shutdown fix. Matching companion candidates are 0.7.4 / 0.7.4-dev.1. See [release notes](docs/releases/v0.8.13.md) for scope and limits. These are local candidates only; publish the companion separately and repeat the final server build before publication. No installed application or live configuration has been changed. Earlier entries describe historical checkpoints.

## Compact Windows endpoint cards — 7 September 2026

Published server Main 0.8.12 (`8ce9e12`) / Development 0.8.12-dev.1 (`d39feeb`), with matching tags and four verified assets per release. Windows endpoint cards start collapsed with hostname, address, connectivity and attention indicators; expanding reveals health, onboarding and removal controls. Native disclosures support mouse and keyboard input, and their state follows the endpoint GUID through background redraws and registration. Newly discovered cards and page reloads start collapsed. Desktop and mobile browser checks confirmed independent card heights, no horizontal overflow, accessible controls and expansion retention across real polling redraws. All 23 server regression checks, 13 frontend checks and the installer suite passed. Both clean release packages passed provenance, embedded ZIP, source stamp, frontend hash and isolated startup checks. They bundle the already published PC Agent Main 0.7.3 / Development 0.7.3-dev.1; no companion source or integration contract changes are included. The standard Main installer completed locally, and the running server and updater confirm 0.8.12. Installed server/frontend hashes match the release, both Windows registrations and all four devices remain, and the job, selected network, NDI configuration hash and PC Agent binary are unchanged. The installed page confirms both Windows cards start collapsed and expand correctly. Detailed package, publication and deployment evidence is under `%LOCALAPPDATA%/Temp/ndi-cards-release-20260907`.
