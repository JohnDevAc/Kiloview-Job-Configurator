...
function remoteOnboardingEditor(agent,registered=false){
  ...
  if(!pcAgentSupportsRemoteOnboarding(agent))return '<div class="local-pc-drift"><strong>PC AGENT UPDATE REQUIRED</strong><span>Install an agent that supports managed remote onboarding and guarded network configuration.</span></div>';
...
function renderMonitor(app){
  ...
  const kiloviewCard=d=>{
    const body=`<h3>${esc(d.hostname)}</h3><div class="ip">${esc(d.ipAddress)}</div><div class="meta"><span class="pill ${roleClass(d.role)}">${esc(d.role)}</span><span class="pill">${esc(d.ndiGroup)}</span>${multicastMeta(d)}${d.firmwareVersion?`<span class="pill">FW ${esc(d.firmwareVersion)}</span>`:''}${d.hdmiOutputResolution?`<span class="pill">${esc(d.hdmiOutputResolution)}</span>`:''}</div>${d.multicastLastError?`<div class="error-text">Multicast: ${esc(d.multicastLastError)}</div>`:''}${d.lastError?`<div class="error-text">${esc(d.lastError)}</div>`:''}<div class="card-actions"><a href="${esc(deviceUrl(d))}" target="_blank" rel="noreferrer">Device UI ↗</a><button class="remove-kiloview" data-kiloview-action="remove" data-device-id="${esc(d.id)}" data-device-name="${esc(d.hostname)}">Remove from job</button></div>`;
    return `<article class="device-card"><header><span class="model">${esc(d.model)} · ${esc(d.role)}</span><span class="card-indicators">${multicastIcon(d)}<i class="health ${roleClass(d.health)}" title="${esc(d.health)}"></i></span></header>${d.role==='Encoder'?encoderPreview(d)+`<div class="encoder-card-body">${body}</div>`:body}</article>`;
  };
