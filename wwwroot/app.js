...
function windowsEndpointPills({statusLabel,statusClass,statusTitle,preferred,adapterName,ndiToolsVersion,jobName,assignment,multicastApplied=true}){
  ...
  return `<span data-connectivity-title class="pill ${esc(statusClass)}"${title}>${esc(statusLabel)}</span><span class="pill ${preferred?'encoder':'standalone'}">${preferred?'PREFERRED NDI INTERFACE':'INTERFACE ATTENTION'}</span>${adapterName?`<span class="pill">${esc(adapterName)}</span>`:''}<span class="pill">NDI ${esc(ndiToolsVersion||'UNKNOWN')}</span>${jobName?`<span class="pill">${esc(jobName)}</span>`:''}${assignment?.netPrefix?`<span class="pill ${multicastApplied?'multicast-pill':'standalone'}">MC ${esc(assignment.netPrefix)}/${prefixLength(assignment.netmask)}</span><span class="pill">TTL ${esc(assignment.ttl)}</span>`:'<span class="pill">UNICAST READY</span>'}`;
}
...
const {windowsEndpointCard,captureWindowsExpansion,restoreWindowsExpansion}=createWindowsCards({esc});
...
function renderOnboardingLocalPcCard(){
  const network=state.selectedNetwork,component=state.localPcComponent,available=component?.compatible===true;
  onboardingLocalPcCard.classList.toggle('configuration-drift',false);
  onboardingLocalPcCard.innerHTML=`<header>${windowsBadge(component?.operatingSystemVersion)}<span class="pill agent-product">SERVER PC</span></header><h3>${esc(component?.hostname||'This Windows PC')}</h3><div class="ip">${network?`${esc(network.address)}/${network.prefixLength}`:'SELECT AN ADAPTER'}</div><div class="meta"><span class="pill">${available?'PC AGENT INSTALLED':'PC AGENT NOT AVAILABLE'}</span>${component?.version?`<span class="pill">${esc(component.version)}</span>`:''}</div><div class="local-pc-note">${available?'The installed onboarding app will apply this job’s NDI settings. The server’s IP address is retained and no additional confirmation is required.':esc(component?.error||'Install the optional PC Agent component from the server installer to include this PC.')}</div><label class="check"><input id="includeServerPc" type="checkbox" ${state.includeServerPc&&available?'checked':''} ${available?'':'disabled'}><span>Onboard this server PC</span></label><div class="card-actions"><button type="button" data-local-onboarding-action="refresh">Check installation</button></div>`;
  $('#includeServerPc').onchange=e=>{state.includeServerPc=e.target.checked;updateSelected()};
  updateSelected();
}
function renderOnboardingPcAgents(){
  const container=$('#onboardingPcAgentCards');if(!container)return;
  const drafts=captureRemoteOnboardingEditors(container),expanded=captureWindowsExpansion(container);
  const agents=(state.pcAgents||[]).filter(agent=>agent.address!==state.selectedNetwork?.address);
  $('#windowsEndpointDiscoveryCount').textContent=`${agents.length+1} ENDPOINT${agents.length?'S':''}`;
  container.innerHTML=agents.map(agent=>windowsEndpointCard(agent,{className:'pc-agent-card',context:agent.registered?'Onboarded':'Not onboarded',body:`<header>${windowsBadge(agent.operatingSystemVersion)}</header><div class="meta"><span class="pill agent-product">${esc(agent.product||PC_AGENT_PRODUCT)}</span><span class="pill">NDI ${esc(agent.ndiToolsVersion||'NOT INSTALLED')}</span><span class="pill">VERSION ${esc(agent.agentVersion)}</span><span class="pill">JOBS ${esc(agent.memberships?.length||0)}</span></div>${remoteOnboardingEditor(agent,agent.registered)}`})).join('');
  restoreWindowsExpansion(container,expanded);
  restoreRemoteOnboardingEditors(container,drafts);
}
async function refreshLocalPcComponent(){
  try{state.localPcComponent=await api('/api/pc-onboarding/local')}
  catch{state.localPcComponent=null}
  renderOnboardingLocalPcCard();
  return state.localPcComponent;
}
...
async function boot(){
  try{const health=await api('/api/health'),development=String(health.channel).toLowerCase()==='development';$('#appVersion').textContent=`v${health.version}`;$('#headerVersion').textContent=`Version v${health.version}${development?' · DEV':''}`;$('#developmentBanner').classList.toggle('hidden',!development);document.body.classList.toggle('development-build',development);$('#serviceDot').className='online';const [app]=await Promise.all([api('/api/state'),loadPcAgents()]);state.devices=app.devices||[];await loadNetworkSelection(app);const remoteOnboarded=(app.windowsPcs||[]).length>0;if(app.lastJob&&(state.devices.some(d=>d.isOnboarded)||remoteOnboarded)){renderMonitor(app);show('monitor')}else show('setup')}
...
async function pollProgress(){clearInterval(state.poll);let finished=false;const tick=async()=>{try{const p=await api('/api/onboarding/progress');renderProgress(p);if(!['idle','running'].includes(p.status)){finished=true;clearInterval(state.poll);state.poll=null;await loadDecoderOrMonitor(p.status)}}catch(e){toast(e.message,true)}};await tick();if(!finished)state.poll=setInterval(tick,1200)}
...
function availableServerPcCard(agent,job){
  return windowsEndpointCard(agent,{className:'pc-agent-card server-onboarding-card',isServerPc:true,context:'Not onboarded',body:`
    <header>${windowsBadge(agent.operatingSystemVersion)}</header>
    ...
    <div class="meta"><span class="pill">NDI ${esc(agent.ndiToolsVersion||'NOT INSTALLED')}</span><span class="pill">AGENT ${esc(agent.agentVersion)}</span></div>${windowsMetrics(agent)}
    ${localOnboarding.render()}`});
}
function renderMonitor(app){
  ...
  const devices=state.devices.filter(d=>d.isOnboarded),teletools=devices.filter(isTeleTool),kiloviews=devices.filter(d=>!isTeleTool(d)),decoderDevices=kiloviews.filter(d=>d.role==='Decoder'),encoderDevices=kiloviews.filter(d=>d.role==='Encoder'),unknownDevices=kiloviews.filter(d=>d.role==='Unknown'),windowsPcs=app.windowsPcs||[],registeredEndpointIds=new Set(windowsPcs.map(pc=>String(pc.endpointId).toLowerCase())),pcAgents=state.pcAgents||[],agentById=new Map(pcAgents.map(agent=>[String(agent.endpointId).toLowerCase(),agent])),availablePcAgents=pcAgents.filter(agent=>!registeredEndpointIds.has(String(agent.endpointId).toLowerCase())),healthyRemotePcs=windowsPcs.filter(pc=>pc.preferredInterfaceConfigured&&pc.status==='onboarded'&&!pc.error&&(agentById.get(String(pc.endpointId).toLowerCase())?.status==='online'||pc.connectivityStatus==='online')),online=devices.filter(d=>d.health==='Online').length+healthyRemotePcs.length,running=teletools.filter(d=>d.streamRunning).length,onboarded=devices.length+windowsPcs.length,benignManagementStates=new Set(['','managed','available','mode-starting','mode-fallback']),deviceAttention=devices.filter(d=>d.health!=='Online'||(!isTeleTool(d)&&d.role==='Unknown')||!!d.lastError||!!d.multicastLastError||!benignManagementStates.has(String(d.managementState||'').toLowerCase())).length,windowsAttention=(windowsPcs.length-healthyRemotePcs.length),needsAttention=deviceAttention+windowsAttention;
  ...
  const windowsJobName=app.lastJob?.jobName||app.multicast?.jobName;
  const remotePcCard=pc=>{
    ...
    const {agentProduct,online,offline,stale,statusLabel,statusClass,connectivityTitle}=windowsConnectivity(pc,agent);
    const assignment=app.multicast?.assignments?.find(item=>item.status!=='unicast'&&String(item.endpointId).toLowerCase()===String(pc.endpointId).toLowerCase());
    ...
    return windowsEndpointCard(pc,{className:`remote-pc-card ${configured&&online?'':'configuration-drift'}`,statusLabel,statusClass,statusTitle:connectivityTitle,context:assignment?'Multicast':'Unicast',needsAttention:!configured||!!manualMulticast,body:`<header>${windowsBadge(pc.operatingSystemVersion)}<span class="card-indicators">${multicastIcon(assignment?{...assignment,multicastConfigured:true}:{})}</span></header><div class="meta">${pills}${agent?`<span class="pill agent-product">${esc(agentProduct)}</span><span class="pill">VERSION ${esc(agent.agentVersion)}</span><span class="pill">JOBS ${esc(agent.memberships?.length||0)}</span>`:''}</div>${metrics}${attention}${manualMulticast}${pc.isServerPc?localOnboarding.render(true):agent?remoteOnboardingEditor(agent,true):''}<div class="card-actions"><button type="button" class="remove-remote-pc" ${pc.isServerPc?'data-local-onboarding-remove':''} data-remote-pc-action="remove" data-endpoint-id="${esc(pc.endpointId)}" data-endpoint-name="${esc(pc.hostname)}">Remove</button></div>`});
  };
  const availablePcAgentCard=agent=>agent.address===state.selectedNetwork?.address?availableServerPcCard(agent,app.lastJob):windowsEndpointCard(agent,{className:'pc-agent-card',context:'Not onboarded',body:`<header>${windowsBadge(agent.operatingSystemVersion)}</header><div class="meta"><span class="pill agent-product">${esc(agent.product||PC_AGENT_PRODUCT)}</span><span class="pill">NDI ${esc(agent.ndiToolsVersion||'NOT INSTALLED')}</span><span class="pill">VERSION ${esc(agent.agentVersion)}</span><span class="pill">JOBS ${esc(agent.memberships?.length||0)}</span></div>${windowsMetrics(agent)}<div class="local-pc-note">Onboarding requires approval on this remote PC.</div>${remoteOnboardingEditor(agent,false)}`});
  ...
  if(cardSignature!==state.monitorCardSignature){
    ...
    restoreWindowsExpansion(monitorGrid,expandedWindows);
...
function renderMulticastPlan(plan){
  ...
  const access=$('#accessManagerStatus');access.className='access-manager-status ready';access.innerHTML='<strong>PC Agent managed endpoints</strong><span>Every onboarded Windows PC, including the server, receives its own allocation through the PC Agent.</span>';
...
async function loadMulticastSetup(regenerate=false){
  const ttl=Math.max(1,Math.min(255,Number($('#multicastTtl').value)||1)),apply=$('#applyMulticast');
  const networkStatus=$('#multicastNetworkAdapter');
  networkStatus.className='access-manager-status ready';networkStatus.textContent=state.selectedNetwork?`${state.selectedNetwork.name} · ${state.selectedNetwork.address}`:'Select a production adapter before applying multicast.';
  ...
  try{
    const plan=await api('/api/multicast/plan',{method:'POST',body:JSON.stringify({ttl,regenerate})});
