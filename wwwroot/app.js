...
function renderPlan(){const hasKiloview=state.plan.devices.some(d=>['N6','N60'].includes(d.family)),hasTeleTool=state.plan.devices.some(d=>['TeleTool','SimulatedTeleTool'].includes(d.family));$('#planRows').innerHTML=state.plan.devices.map(d=>`<div class="plan-row"><div><small>${['TeleTool','SimulatedTeleTool'].includes(d.family)?'TELETOOL':'KILOVIEW'} · FOUND ADDRESS</small><span class="from">${esc(d.currentIp)}</span></div><div class="arrow">→</div><div><small>STATIC ADDRESS</small><span class="to">${esc(d.targetIp)}</span></div><div><small>INITIAL HOSTNAME</small><strong>${esc(d.hostname)}</strong></div></div>`).join('');$('#planWarnings').innerHTML=state.plan.warnings.map(w=>`<div class="warning">⚠ ${esc(w)}</div>`).join('');$('#planCount').textContent=`${state.plan.devices.length} device${state.plan.devices.length===1?'':'s'} will be changed`;$('#planConfirmation').textContent=state.settings.cleanOnboarding?'Clean onboarding will permanently clear prior KiloLink and configurator job inventory before this plan starts.':hasKiloview?'Firmware is staged next; EULA acceptance applies only to selected Kiloview units.':'TeleTools will be configured through their Dev management API; no Kiloview EULA action is required.';$('#runPlan').querySelector('span').textContent=hasKiloview?hasTeleTool?'Continue to firmware for Kiloviews':'Continue to firmware':'Onboard TeleTools'}
async function startOnboarding(){try{$('#runPlan').disabled=true;await api('/api/onboarding/run',{method:'POST',body:JSON.stringify({planId:state.plan.planId})});show('progress');pollProgress()}catch(e){toast(e.message,true);$('#runPlan').disabled=false;throw e}}
$('#runPlan').onclick=async()=>{const ids=new Set(state.plan.devices.filter(d=>['N6','N60'].includes(d.family)).map(d=>d.deviceId)),devices=(state.discovery?.devices||state.devices).filter(d=>ids.has(d.id));if(devices.length){state.firmwarePreOnboarding=true;renderFirmware({devices});show('firmware')}else await startOnboarding()};
...
async function loadDecoderOrMonitor(status){const app=await api('/api/state');state.devices=app.devices||[];const onboarded=state.devices.filter(d=>d.isOnboarded),runDevices=currentRunDevices(onboarded),kiloviews=runDevices.filter(d=>!isTeleTool(d)),decoders=kiloviews.filter(d=>d.role==='Decoder'),encoders=kiloviews.filter(d=>d.role==='Encoder');if(kiloviews.length){renderDecoders(decoders,encoders);show('decoder');if(decoders.length)await prepareIdentityCards();if(status==='failed')toast('Onboarding finished with errors; successfully onboarded displays can still be named.',true)}else{renderMonitor(app);show('monitor')}}
...
async function openDisplayNaming(){try{const app=await api('/api/state');state.devices=app.devices||[];state.plan=null;const kiloviews=state.devices.filter(d=>d.isOnboarded&&!isTeleTool(d)),decoders=kiloviews.filter(d=>d.role==='Decoder'),encoders=kiloviews.filter(d=>d.role==='Encoder');if(!decoders.length){toast('No onboarded Kiloview decoders are available.',true);return}renderDecoders(decoders,encoders);show('decoder');await prepareIdentityCards()}catch(err){toast(`Display naming: ${err.message}`,true)}}
...
function renderMonitor(app){
  ...
  const devices=state.devices.filter(d=>d.isOnboarded),teletools=devices.filter(isTeleTool),kiloviews=devices.filter(d=>!isTeleTool(d)),decoderDevices=kiloviews.filter(d=>d.role==='Decoder'),encoderDevices=kiloviews.filter(d=>d.role==='Encoder'),remoteWindowsPcs=app.remoteWindowsPcs||[],healthyRemotePcs=remoteWindowsPcs.filter(pc=>pc.preferredInterfaceConfigured&&pc.status==='onboarded'&&!pc.error&&pc.connectivityStatus==='online'),localAssignment=app.multicast?.assignments?.find(a=>a.endpointId==='local-pc'),localPc=app.localPc||(localAssignment?{...localAssignment,preferredInterfaceConfigured:true,adapterName:'Previously selected adapter'}:null),preferredApplied=localPc?.preferredInterfaceConfigured===true,localMulticastApplied=!localAssignment||localAssignment.status==='applied',localHealthy=preferredApplied&&localMulticastApplied,online=devices.filter(d=>d.health==='Online').length+(localHealthy?1:0)+healthyRemotePcs.length,running=teletools.filter(d=>d.streamRunning).length,onboarded=devices.length+(localPc?1:0)+remoteWindowsPcs.length;
  $('#nameDisplays').classList.toggle('hidden',!decoderDevices.length);
  ...
  const windowsJobName=app.lastJob?.jobName||app.multicast?.jobName,localError=localPc?.error||localAssignment?.error,localStatusLabel=localHealthy?'ONLINE':'ATTENTION',localStatusClass=localHealthy?'connectivity-online':'connectivity-offline',localPills=localPc?windowsEndpointPills({statusLabel:localStatusLabel,statusClass:localStatusClass,statusTitle:localHealthy?'Local endpoint ready':'Local endpoint needs attention',preferred:preferredApplied,adapterName:localPc.adapterName,ndiToolsVersion:localPc.ndiToolsVersion,jobName:windowsJobName,assignment:localAssignment,multicastApplied:localMulticastApplied}):'',localCard=localPc?`<article class="device-card local-pc-card ${localHealthy?'':'configuration-drift'}"><header><span class="model">${windowsModel(localPc.operatingSystemVersion)}</span><span class="card-indicators">${multicastIcon(localAssignment||{})}<i class="health ${localHealthy?'online':'error'}" title="${localHealthy?'Applied':'Configuration changed'}"></i></span></header><h3>${esc(localPc.hostname)}</h3><div class="ip">${esc(localPc.address)}/${esc(localPc.prefixLength)}</div><div class="meta">${localPills}</div>${localError?`<div class="local-pc-drift"><strong>NDI Access Manager needs attention</strong><span>${esc(localError)}</span></div>`:`<div class="local-pc-note">${localAssignment?'Preferred interface and multicast settings match NDI Access Manager.':'Preferred interface is applied. Multicast setup will update this same endpoint card.'}</div>`}${!preferredApplied?'<div class="card-actions"><button type="button" data-local-pc-action="reapply">Reapply preferred interface</button></div>':''}</article>`:'';
  const remotePcCard=pc=>{
    const configured=pc.preferredInterfaceConfigured&&pc.status==='onboarded'&&!pc.error,status=String(pc.connectivityStatus||'unknown').toLowerCase(),online=status==='online',offline=status==='offline',stale=status==='stale',statusLabel=online?'ONLINE':offline?'OFFLINE':'CHECKING',healthClass=online?'online':offline?'error':'configuring',statusClass=online?'connectivity-online':offline?'connectivity-offline':stale?'connectivity-stale':'connectivity-unknown',lastReply=pc.lastSeenUtc?new Date(pc.lastSeenUtc).toLocaleString():'not yet recorded',connectivityTitle=online?`Reachable · last reply ${lastReply}`:offline?`No reply to three consecutive checks · last reply ${lastReply}`:stale?`No reply to ${pc.consecutiveConnectivityFailures||1} recent check${pc.consecutiveConnectivityFailures===1?'':'s'} · last reply ${lastReply}`:'Waiting for the first connectivity check',assignment=app.multicast?.assignments?.find(item=>String(item.endpointId).toLowerCase()===String(pc.endpointId).toLowerCase()),manualMulticast=assignment?.netPrefix?`<div class="remote-multicast-setup"><strong>Manual NDI Access Manager setup</strong><span>On ${esc(pc.hostname)}, enable Multicast Send and Receive, then enter:</span><dl><div><dt>NET PREFIX</dt><dd>${esc(assignment.netPrefix)}</dd></div><div><dt>NETMASK</dt><dd>${esc(assignment.netmask)}</dd></div><div><dt>TTL</dt><dd>${esc(assignment.ttl)}</dd></div></dl><small>This range is reserved for this PC. The Job Configurator cannot apply or verify it remotely.</small></div>`:'',attention=pc.error?`<div class="local-pc-drift"><strong>Windows endpoint needs attention</strong><span>${esc(pc.error)}</span></div>`:offline?`<div class="local-pc-drift"><strong>Windows endpoint offline</strong><span>No ICMP reply was received on three consecutive checks. Confirm the PC is powered on and its onboarding firewall rule is enabled.</span></div>`:stale?`<div class="remote-pc-checking"><strong>Confirming connectivity</strong><span>A recent check did not receive a reply. The endpoint is marked offline only after three consecutive failures.</span></div>`:`<div class="local-pc-note">Preferred interface, job group, and discovery server were applied by PC Onboarding v${esc(pc.utilityVersion||'unknown')}.</div>`;
