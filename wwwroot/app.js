...
function renderOnboardingPcAgents(){
  ...
  container.innerHTML=agents.map(agent=>`<article class="device-card pc-agent-card"><header><span class="model">${windowsModel(agent.operatingSystemVersion)}</span><i class="health online" title="PC Agent online"></i></header><h3>${esc(agent.hostname)}</h3><div class="ip">${esc(agent.address)}/${esc(agent.prefixLength)}</div><div class="meta"><span class="pill connectivity-online">AGENT ONLINE</span><span class="pill">${esc(agent.adapterName)}</span><span class="pill">NDI ${esc(agent.ndiToolsVersion||'NOT INSTALLED')}</span><span class="pill">AGENT ${esc(agent.agentVersion)}</span><span class="pill">JOBS ${esc(agent.memberships?.length||0)}</span>${agent.registered?'<span class="pill encoder">ONBOARDED</span>':'<span class="pill standalone">AVAILABLE</span>'}</div>${agent.registered?'':`<div class="card-actions"><button type="button" class="onboard-pc-agent" data-pc-agent-action="onboard" data-endpoint-id="${esc(agent.endpointId)}" data-endpoint-name="${esc(agent.hostname)}">Onboard this PC</button></div>`}</article>`).join('');
}
...
function renderMonitor(app){
  ...
  const remotePcCard=pc=>{
    const agent=agentById.get(String(pc.endpointId).toLowerCase()),configured=pc.preferredInterfaceConfigured&&pc.status==='onboarded'&&!pc.error,status=String(pc.connectivityStatus||'unknown').toLowerCase(),online=status==='online',offline=status==='offline',stale=status==='stale',statusLabel=online?'ONLINE':offline?'OFFLINE':'CHECKING',healthClass=online?'online':offline?'error':'configuring',statusClass=online?'connectivity-online':offline?'connectivity-offline':stale?'connectivity-stale':'connectivity-unknown',lastReply=pc.lastSeenUtc?new Date(pc.lastSeenUtc).toLocaleString():'not yet recorded',connectivityTitle=online?`PC Agent reachable · last status ${lastReply}`:offline?`PC Agent failed three consecutive status polls · last status ${lastReply}`:stale?`PC Agent missed ${pc.consecutiveConnectivityFailures||1} recent status poll${pc.consecutiveConnectivityFailures===1?'':'s'} · last status ${lastReply}`:'Waiting for the first PC Agent status poll',assignment=app.multicast?.assignments?.find(item=>String(item.endpointId).toLowerCase()===String(pc.endpointId).toLowerCase()),manualMulticast=assignment?.netPrefix?`<div class="remote-multicast-setup"><strong>Manual NDI Access Manager setup</strong><span>On ${esc(pc.hostname)}, enable Multicast Send and Receive, then enter:</span><dl><div><dt>NET PREFIX</dt><dd>${esc(assignment.netPrefix)}</dd></div><div><dt>NETMASK</dt><dd>${esc(assignment.netmask)}</dd></div><div><dt>TTL</dt><dd>${esc(assignment.ttl)}</dd></div></dl><small>This range is reserved for this PC. The Job Configurator cannot apply or verify it remotely.</small></div>`:'',attention=pc.error?`<div class="local-pc-drift"><strong>Windows endpoint needs attention</strong><span>${esc(pc.error)}</span></div>`:offline?`<div class="local-pc-drift"><strong>Windows endpoint offline</strong><span>The PC Agent did not answer three consecutive authenticated-contract status checks on TCP 8094.</span></div>`:stale?`<div class="remote-pc-checking"><strong>Confirming agent connectivity</strong><span>A recent PC Agent status poll failed. The endpoint is marked offline only after three consecutive failures.</span></div>`:`<div class="local-pc-note">PC Agent ${esc(pc.agentVersion||agent?.agentVersion||'unknown')} is reporting live Windows and NDI health.</div>`,metrics=`<div class="pc-agent-metrics"><span>AGENT UP ${compactDuration(pc.agentUptimeSeconds||agent?.agentUptimeSeconds)}</span><span>PC UP ${compactDuration(pc.machineUptimeSeconds||agent?.machineUptimeSeconds)}</span><span>MEM FREE ${compactBytes(pc.physicalMemoryAvailableBytes||agent?.physicalMemoryAvailableBytes)} / ${compactBytes(pc.physicalMemoryTotalBytes||agent?.physicalMemoryTotalBytes)}</span><span>DISK FREE ${compactBytes(pc.systemDriveFreeBytes||agent?.systemDriveFreeBytes)} / ${compactBytes(pc.systemDriveTotalBytes||agent?.systemDriveTotalBytes)}</span></div>`;
...
async function openPcAgentOnboarding(button){
  const endpointId=button.dataset.endpointId,endpointName=button.dataset.endpointName||'the Windows PC';
  try{
    button.disabled=true;button.textContent='Waiting for approval…';
    await api(`/api/pc-agents/${encodeURIComponent(endpointId)}/onboarding/open`,{method:'POST',body:'{}'});
    toast(`${endpointName} approved the request; its elevated onboarding utility is opening`);
  }catch(err){toast(err.message,true)}
  finally{button.disabled=false;button.textContent='Onboard this PC'}
}
...
document.addEventListener('click',e=>{const button=e.target.closest('[data-pc-agent-action="onboard"]');if(button){e.preventDefault();openPcAgentOnboarding(button)}});
