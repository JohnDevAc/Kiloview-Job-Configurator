...
async function loadDecoderOrMonitor(status){const app=await api('/api/state');state.devices=app.devices||[];const onboarded=state.devices.filter(d=>d.isOnboarded),runDevices=currentRunDevices(onboarded),kiloviews=runDevices.filter(d=>!isTeleTool(d)),decoders=kiloviews.filter(d=>d.role==='Decoder'),encoders=kiloviews.filter(d=>d.role==='Encoder');if(kiloviews.length&&status!=='failed'){renderDecoders(decoders,encoders);show('decoder');if(decoders.length)await prepareIdentityCards()}else{renderMonitor(app);show('monitor')}}
