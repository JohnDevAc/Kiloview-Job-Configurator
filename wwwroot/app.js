...
function renderMonitor(app){
  ...
  const remotePcCard=pc=>{const healthy=pc.preferredInterfaceConfigured&&pc.status==='onboarded'&&!pc.error;return `<article class="device-card remote-pc-card ${healthy?'':'configuration-drift'}"><header><span class="model">WINDOWS PC · NDI TOOLS</span><i class="health ${healthy?'online':'error'}" title="${healthy?'Onboarded':'Needs attention'}"></i></header><h3>${esc(pc.hostname)}</h3><div class="ip">${esc(pc.address)}/${esc(pc.prefixLength)}</div><div class="meta"><span class="pill ${healthy?'encoder':'standalone'}">${healthy?'ONBOARDED':'NEEDS ATTENTION'}</span><span class="pill">${esc(pc.adapterName)}</span><span class="pill">NDI ${esc(pc.ndiToolsVersion||'UNKNOWN')}</span></div>${pc.error?`<div class="local-pc-drift"><strong>Windows endpoint needs attention</strong><span>${esc(pc.error)}</span></div>`:`<div class="local-pc-note">Preferred interface, job group, and discovery server were applied by PC Onboarding v${esc(pc.utilityVersion||'unknown')}.</div>`}</article>`};
  const cardDevices=devices.map(({lastSeenUtc,...device})=>device),cardSignature=JSON.stringify({devices:cardDevices,localPc,localAssignment,remoteWindowsPcs,jobName:app.multicast?.jobName});
