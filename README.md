...
1. Select the active IPv4 adapter connected to the device network. This stores the network used by discovery and management; it does not write NDI settings. The **Windows NDI Endpoints** card offers **Onboard this server PC**, selected by default when the optional PC Agent component is installed. The companion retains the server IP and applies NDI settings when the plan starts. NDI Access Manager, NDI Discovery and Studio Monitor must be closed; the background NDI Discovery Server remains running. The Service connections section then starts minimized with a live status spinner while the application scans only the selected network for KiloLink Server Pro and an NDI Discovery Server listening on TCP `5959`, then verifies KiloLink authentication. A green tick keeps the section minimized when everything is ready; any discovery or login failure expands it for intervention, and a manual Show details/Minimize control remains available. Existing KiloLink credentials are reused from Windows Credential Manager. When no stored credential works, the application checks the official KiloLink Server Pro factory login `admin/Kiloview001`; a factory server is given the exact Job Name as its new administrator password, verified, and stored securely before onboarding continues. KiloLink requires that Job Name to be 6–32 characters with an uppercase letter, lowercase letter, and number.
...
Remote Windows NDI endpoints use an **NDI Configurator PC Agent** that advertises `remote-onboarding-v2` and `network-config-v1`. During migration, the server accepts both `NDI Configurator PC Agent` and the legacy `Kiloview PC Agent` product value, but always displays the new name. The Job Configurator retains the exact legacy-compatible `KILOVIEW_PC_AGENT_DISCOVER_V1` datagram and sends it by UDP unicast to the bounded selected-adapter scan range on port `8093`. It validates the response address, compatible product, schema, endpoint GUID and capabilities, then confirms `GET /api/health` on TCP `8094`. Duplicate replies are reconciled only by stable `endpointId`; product text is never used as device identity. A discovered agent remains transient until the existing registration API succeeds. The monitor reads `/api/v1/status` every 15 seconds with a three-second timeout. A card remains green while status is available, turns amber after one missed poll, and red after three. ICMP/agentless monitoring is no longer used.
...
Server and remote Windows PCs use the same agent-backed cards, live metrics, drift checks, and multicast controls. The server carries a **SERVER PC** badge and a **Reapply server NDI settings** action. A plan can onboard only the server PC without selecting physical devices.
...
On Windows, the PC Agent owns NDI configuration and backups. The server sends membership-authorized multicast requests to every onboarded Windows endpoint, including itself. The agent preserves unrelated settings and reports readback/drift; the server reloads its preview runtime after a local change. Close Access Manager and active NDI clients before applying multicast.
...
The server workspace owns both codebases while keeping their Git repositories, versions, installers and release feeds separate. Open `NDI-Configurator.code-workspace`; `suite.json` locates the sibling `Kiloview PC Onboarding` checkout. See [PC-ONBOARDING-CONTRACT.md](PC-ONBOARDING-CONTRACT.md) for ownership, package provenance, installation and both onboarding paths.

The installed companion owns all Windows NDI writes. Server-local onboarding invokes its elevated Setup through a bounded stdin/stdout JSON contract without another confirmation. Remote PCs retain their local approval and UAC flow. All PCs register under the same `WindowsPcs` collection and stable agent GUID, then use common monitoring and multicast. Selecting a network alone does not register a PC. Legacy local/remote PC state is not migrated: re-onboard Windows PCs after upgrading.
...
agents show **NDI Configurator PC Agent update required** and the legacy local UI is never opened.
...
Windows endpoint. Agents advertising `multicast-config-v1` receive an
...
The companion must be checked out separately at the path in `suite.json`, with version 0.7.0 or later. The publish script invokes both repositories’ own builds and bundles the complete companion package with a version/commit/SHA-256 manifest. Override its location with `-CompanionRoot`. Tagged release builds require a clean companion checkout.
...
Extract `artifacts\NDI-Job-Configurator-Windows.zip` and run `Install.cmd`; it offers the same default-selected PC Agent option. For an explicit scripted opt-out, run `Install-NDIJobConfigurator.ps1 -SkipPcAgent`. Installation is per-user, registers an elevated scheduled task so the service starts with administrator rights at sign-in, and creates branded Desktop and Start Menu launch shortcuts. The installer and application request elevation through Windows UAC.
...
- NDI Configurator PC Agent discovery and monitoring are read-only, bound to the selected adapter/subnet, and fixed to UDP `8093` and TCP `8094`. The Job Configurator does not scan outside its existing bounded IPv4 range, trust an advertised address different from the datagram source, or expose remote NDI editing, software installation, membership removal, agent control, UAC bypass, or command execution. Remote onboarding requires local approval and UAC. On the server itself, the installed elevated companion applies the selected job directly after initial component installation.
...
- The selected network-adapter ID and current IPv4 address are stored in the same application state and reused by every network-aware workflow. When the server PC is onboarded, the companion stores its address as the sole NDI preferred interface. Agent status detects changed interfaces, job groups, or discovery-server settings and marks the endpoint for attention.
...
- Multicast allocations are conflict-checked within the generated organization-local pool. The companion backs up and validates Windows NDI configuration; the server has no direct local-PC configuration writer.
